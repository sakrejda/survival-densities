...
N <- 10^3
lt_delays <- flexsurv::rgengamma.orig(n=N, shape=.4, scale=1, k=16)
lt_delays <- lt_delays/median(lt_delays)
...
compiled_generalized_gamma <- stan_model(file='../../models/stan-lang/full/generalized-gamma-p1.stan', 
...
pl_g_st_delays <- ggplot(data=data %>% filter(tail=='short'), 
  ...
  scale_x_continuous("delay", limits=c(0,5.5))
for (i in sample(x=1001:2000, size=10)) 
  pl_g_st_delays <- pl_g_st_delays + stat_function(fun=dgamma, alpha=.5, 
    args=list(shape=g_g_alpha[i], scale=g_g_beta[i]))
...
pl_g_lt_delays <- ggplot(data=data %>% filter(tail=='long'), 
  ...
  scale_x_continuous("delay", limits=c(0,5.5))
for (i in sample(x=1001:2000, size=10)) 
  pl_g_lt_delays <- pl_g_lt_delays + stat_function(fun=dgamma, alpha=.5, 
    args=list(shape=g_gg_alpha[i], scale=g_gg_beta[i]))
...
st_delays_tq <- st_delays_q
for (i in seq_along(delay_quantiles)) {
  st_delays_tq[i] <- qgamma(p=delay_quantiles[i],
    shape=mean(g_g_alpha), scale=mean(g_g_beta))
}

lt_delays_tq <- lt_delays_q
for (i in seq_along(delay_quantiles)) {
  lt_delays_tq[i] <- qgamma(p=delay_quantiles[i],
    shape=mean(g_gg_alpha), scale=mean(g_gg_beta))
}

quantiles <- rbind(st_delays_q, st_delays_tq, lt_delays_q, lt_delays_tq) %>%
...
pl_g_st_lt_qq_comparison <- ggplot(data=quantiles, 
...
st_delays_tq <- st_delays_q
for (i in seq_along(delay_quantiles)) {
  st_delays_tq[i] <- flexsurv::qgengamma.orig(p=delay_quantiles[i],
    shape=mean(gg_g_nu), scale=mean(gg_g_beta), k=mean(gg_g_alpha/gg_g_nu))
}

lt_delays_tq <- lt_delays_q
for (i in seq_along(delay_quantiles)) {
  lt_delays_tq[i] <- flexsurv::qgengamma.orig(p=delay_quantiles[i],
...
pl_gg_st_lt_qq_comparison <- ggplot(data=quantiles, 
  aes(x=data, y=model, colour=percentile)
) + geom_point() +
    facet_wrap( ~ source, ncol=1) +
    geom_abline(slope=1, intercept=0) + 
...
# Four-way comparison:
pdf(file='model-st-lt-comparison.pdf', width=6, height=8)
gridExtra::grid.arrange(pl_g_st_delays, pl_g_lt_delays,
  pl_g_st_delays, pl_g_lt_delays, ncol=2)
dev.off()
